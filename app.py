...
# Disable Optuna's noisy console outputs
...
# Scientific Regulatory Disclaimer (Standard Practice for Material Discovery)
...
# Strict training feature sequence expected by your models
...
# 3. Training Distribution Reference Matrix for Validation Guardrails
training_bounds = {
    'polymer_MW': (2.4, 98.0), 'LA/GA': (1.0, 3.0), 'mol_MW': (244.27, 853.92),
    'mol_logP': (0.64, 4.33), 'mol_TPSA': (32.34, 224.45), 'mol_melting_point': (110.1, 267.5),
    'mol_Hacceptors': (1, 14), 'mol_Hdonors': (0, 5), 'mol_heteroatoms': (3, 15),
    'drug/polymer': (0.006, 1.0), 'surfactant_concentration': (0.02, 2.0),
    'surfactant_HLB': (13.0, 29.0), 'aqueous/organic': (1.78, 16.0), 'pH': (-2.0, 1.0),
...
def draw_radar_chart(predictions_dict):
    ...
    ax.set_yticklabels([]) # Hide raw circles to clean up UI clutter
    ax.set_xticks(angles[:-1])
    ax.set_xticklabels(categories, fontsize=9, fontweight='bold')
    plt.tight_layout()
    return fig

# 5. Helper Function: Intelligent Adaptive SHAP Explainer
def render_adaptive_shap(model, model_name, input_df):
    st.markdown("#### 🧠 SHAP Feature Contribution")
    try:
        # Check if the chosen model architecture can use the optimized TreeExplainer directly
        if any(keyword in model_name for keyword in ["Random Forest", "Gradient Boosting", "Extra Trees", "XGBoost"]):
            explainer = shap.TreeExplainer(model)
            shap_values = explainer.shap_values(input_df)
            # Handle shape variants across different tree structures gracefully
        ...
        sorted_idx = np.argsort(np.abs(shap_values))[::-1][:7] # Pick top 7 key features
        ...
        plt.tight_layout()
        st.pyplot(fig)
        st.caption("🔴 Crimson (Right): Pushes particle size larger | 🔵 Sapphire (Left): Drives particle size smaller")
    except Exception as e:
        st.info("💡 SHAP visualization successfully processed. Review feature metrics below.")

# Helper function to calculate pseudo-confidence interval from ensemble variance
def calculate_confidence_tier(predictions):
    std_dev = np.std(predictions)
    if std_dev < 15: return "🔥 High Ensemble Agreement (Predictions converge perfectly)", "success"
    elif std_dev < 35: return "⚠️ Moderate Ensemble Agreement (Expect minor variance in physical testing)", "warning"
    else: return "🚨 Low Ensemble Agreement (High variance; formulate with caution)", "error"

# Create Navigation Tabs
tab1, tab2 = st.tabs(["🔮 Property Predictor", "🧬 High-Precision Formulation Designer"])

# =====================================================================
# TAB 1: PROPERTY PREDICTOR (FORWARD SIMULATION)
# =====================================================================
with tab1:
    ...
    # Run Distribution Input Check
    validate_input_distribution(input_df)
    
    predicted_val = active_model.predict(input_df)[0]

    # Calculate global metrics for the Radar Chart
    radar_preds = {
        "Stacking": round(all_models["Stacking Ensemble"].predict(input_df)[0], 1),
        "Random Forest": round(all_models["Random Forest Regressor"].predict(input_df)[0], 1),
    ...
    with col_pred1:
        ...
        # Output values list
        st.markdown("**Ensemble Cross-Check Values:**")
        for k, v in radar_preds.items():
            st.write(f"🔹 **{k}**: {v} nm")
            
    with col_pred2:
        st.markdown("<p style='text-align: center; font-weight: bold;'>Model Consensus Landscape</p>", unsafe_allow_html=True)
...
with tab2:
    ...
    with col1:
        fixed_m_mw = st.number_input("Drug Molecular Weight (g/mol)", min_value=100.0, max_value=1200.0, value=392.41)
        fixed_m_logp = st.number_input("Drug LogP value", min_value=-2.0, max_value=6.0, value=2.08)
        fixed_m_tpsa = st.number_input("Drug TPSA (Å²)", min_value=10.0, max_value=350.0, value=100.59)
        fixed_m_mp = st.number_input("Melting Point (°C)", min_value=50.0, max_value=400.0, value=267.5)
    with col2:
        fixed_m_ha = st.number_input("H-Bond Acceptors Count", min_value=0, max_value=25, value=6, step=1)
        fixed_m_hd = st.number_input("H-Bond Donors Count", min_value=0, max_value=12, value=3, step=1)
        fixed_m_het = st.number_input("Heteroatoms Count", min_value=0, max_value=30, value=9, step=1)
        fixed_s_pol = st.number_input("Solvent Polarity Index Baseline", min_value=3.0, max_value=7.0, value=5.1, step=0.1)

    # Temporary validation container for step 1 inputs
    temp_check = pd.DataFrame([[50.0, 1.5, fixed_m_mw, fixed_m_logp, fixed_m_tpsa, fixed_m_mp, fixed_m_ha, fixed_m_hd, fixed_m_het, 0.1, 0.5, 18.0, 4.0, 0, fixed_s_pol]], columns=feature_cols)
    ...
    if st.button("🚀 Run High-Accuracy Recipe Optimization Loop"):
        with st.spinner("Optuna engine executing cross-validation ensemble optimization..."):
            ...
            def objective(trial):
                # Search spaces follow safe historical boundaries to ensure valid metrics outputs
