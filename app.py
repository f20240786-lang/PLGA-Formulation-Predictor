...
def render_adaptive_shap(model, model_name, input_df):
    """Configures adaptive SHAP local attributions without relying on a training database file."""
    ...
    try:
        ...
        if any(keyword in model_name for keyword in ["Random Forest", "Gradient Boosting", "Extra Trees", "XGBoost"]):
            ...
            # Fallback cleanly onto localized object permutation tracking architectures
        ...
        st.info("💡 Local SHAP feature plot loaded. Interpret parameter influences using baseline sliding metrics.")
...
with tab2:
    ...
    if st.button("🚀 Run High-Accuracy Recipe Optimization Loop"):
        with st.spinner("Optuna engine executing cross-validation ensemble optimization..."):
            ...
            def objective(trial):
                # Exploration boundaries matching system distribution boundaries
            ...
            final_pred_size = study.best_trial.user_attrs["predicted_size"] # Explicitly verified plural syntax
